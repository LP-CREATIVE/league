...
async function connectToServer(guildId: string, voiceChannelId: string) {
  ...
  connection.on('error', error => {
    log.error({ guildId, error }, "Voice connection error");
  });
  
  connection.on('stateChange', (oldState, newState) => {
    log.info({ guildId, oldStatus: oldState.status, newStatus: newState.status }, "Connection state changed");
  });
  
  try {
    await entersState(connection, VoiceConnectionStatus.Ready, 30_000);
    log.info({ guildId }, "Voice connection is ready");
  } catch (error) {
    log.error({ guildId, error }, "Voice connection failed to become ready");
    connection.destroy();
    return;
  }
  ...
  try {
    ...
    log.error({ guildId, error }, "Failed to setup voice receiver");
  }
